...
After running Dijkstra for each graph size defined in the dataset specifications and recording execution time and memory usage, the representative results are presented in Table~\ref{tab:lab4-dijkstra}.

\begin{table}[H]
    \caption{Table 2.1 -- Dijkstra benchmark at 1500 nodes}
    \label{tab:lab4-dijkstra}
    \centering
    {\fontsize{11}{11}\selectfont\renewcommand{\arraystretch}{1}\input{generated/lab4_dijkstra_table.tex}}
\end{table}

According to Table~\ref{tab:lab4-dijkstra}, the optimized implementation is substantially faster than the classic one. On the dense graph with 1500 nodes and 1461720 edges, the classic version required \(4562.149\) ms, while the optimized version required only \(226.503\) ms. This difference is presented graphically in Figure~\ref{fig:lab4-dijkstra}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../../Lab4/plots/dijkstra_sparse_dense_performance.png}
    \caption{Figure 2.1 -- Dijkstra performance on sparse and dense graphs}
    \label{fig:lab4-dijkstra}
\end{figure}

According to Figure~\ref{fig:lab4-dijkstra}, the difference between the two implementations grows as the number of nodes increases. The logarithmic scale is required because the gap becomes very large on the bigger graphs.
...
After executing Floyd--Warshall for each graph size from the second input range, the representative values are presented in Table~\ref{tab:lab4-floyd}.

\begin{table}[H]
    \caption{Table 2.2 -- Floyd--Warshall benchmark at 125 nodes}
    \label{tab:lab4-floyd}
    \centering
    {\fontsize{11}{11}\selectfont\renewcommand{\arraystretch}{1}\input{generated/lab4_floyd_table.tex}}
\end{table}

According to Table~\ref{tab:lab4-floyd}, the difference between the classic and optimized Floyd--Warshall variants is not as pronounced as for Dijkstra. On dense graphs, the classic version remained faster, while on sparse graphs the optimized version produced a marginal improvement. The corresponding graph is presented in Figure~\ref{fig:lab4-floyd}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\textwidth]{../../Lab4/plots/floyd_warshall_sparse_dense_performance.png}
    \caption{Figure 2.2 -- Floyd--Warshall performance on sparse and dense graphs}
    \label{fig:lab4-floyd}
\end{figure}

According to Figure~\ref{fig:lab4-floyd}, the strong growth of running time relative to the number of nodes is clearly visible. Even though the tested node counts are much smaller than for Dijkstra, the increase is already significant due to the cubic complexity of the method.
